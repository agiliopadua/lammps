...
void FixLangevinDrude::setup(int vflag)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) { // Only the cores need to be in the group.
      if (ref[i] == 0) // Non-polarizable atom
          dof_core_loc++;
      else {
          if (drude[i]) continue;
          dof_core_loc++;
...
void FixLangevinDrude::post_force(int vflag){ 
  ...
  /*for (int i = 0; i < nlocal; i++){ // Check comm_style brick/drude
    if (drude[i]) {
        ...
        if (j < 0) std::cout << "CORE n°" << ref[i] << " missing on proc " << 
        comm->me << " where is its DRUDE (" << atom->tag[i] << ").\n";
        else if (j >= nlocal) std::cout << "CORE n°" << ref[i] << " is a ghost on proc " << 
        comm->me << " whereas its DRUDE (" << atom->tag[i] << ") is owned.\n";
    } else if (ref[i]) {
        int j = atom->map(ref[i]);
        if (j < 0) std::cout << "DRUDE n°" << ref[i] << " missing on proc " << 
        comm->me << " where is its CORE (" << atom->tag[i] << ").\n";
        else if (j >= nlocal) std::cout << "DRUDE n°" << ref[i] << " is a ghost on proc " << 
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) { // only the cores need to be in the group
      if (ref[i] == 0) { // Non-polarizable atom
        ...
        if (drude[i]) continue; // Done together with the core
